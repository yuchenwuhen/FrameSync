...
public class ServerTcp {
	...
	public void StartServer(){
		...
		try {
            ...
            //临时处理 解决套接字只能使用一次问题
            IPEndPoint endPoint = new IPEndPoint(ip, ServerConfig.servePort); //监听的ip地址和端口号
            TcpListener tcpListener = new TcpListener(endPoint);//tcp监听绑定要监听的ip地址和端口号
            tcpListener.Stop();//关闭该ip地址和该端口的Socket；
